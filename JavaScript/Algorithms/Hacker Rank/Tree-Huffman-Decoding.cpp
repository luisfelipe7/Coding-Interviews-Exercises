...
string creatingWord(node *root,node *actual,string s,string word){
     if(s.length()==0){
        return word;
    }else{
        char element=s[0];   
        ...
        return creatingWord(root,actual,newS,word);
    }
}

void decode_huff(node * root, string s) {
    node *tempNode = root;
    string createdWord = creatingWord(root,tempNode,s,"");
